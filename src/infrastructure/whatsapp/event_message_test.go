...
func TestBuildEventPayloadIncludesIsFromMe(t *testing.T) {
	...
	if value, ok := payload["IsFromMe"]; !ok {
		t.Fatalf("expected IsFromMe in payload")
	} else if isFromMe, ok := value.(bool); !ok || !isFromMe {
		t.Fatalf("expected IsFromMe=true, got %v", value)
	}
}
...
func TestBuildEventPayloadReactionIncludesTargetMessageID(t *testing.T) {
	...
	if got := payload["ReactedMessageID"]; got != "MSG100" {
		t.Fatalf("expected ReactedMessageID MSG100, got %v", got)
	}
}
...
func TestBuildEventPayloadReactionWithoutKeyDoesNotPanic(t *testing.T) {
	...
	if _, ok := payload["ReactedMessageID"]; ok {
		t.Fatalf("expected no ReactedMessageID when reaction key is missing")
	}
}
...
func TestBuildEventPayloadImageWithCaption(t *testing.T) {
    ...
    if payload["Message"] != "Check this out!" {
		t.Fatalf("expected Message='Check this out!', got %v", payload["Message"])
	}
}
...
func TestBuildEventPayloadVideoWithCaption(t *testing.T) {
    ...
    if payload["Message"] != "Watch this video" {
		t.Fatalf("expected Message='Watch this video', got %v", payload["Message"])
	}
}
...
func TestBuildEventPayloadDocumentWithCaption(t *testing.T) {
    ...
    if payload["Message"] != "Important document" {
		t.Fatalf("expected Message='Important document', got %v", payload["Message"])
	}
}
...
func TestBuildEventPayloadQuotedBodyUsesQuotedCaption(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
            ...
            assert.Equal(t, "QuoteMessage", payload["Type"])
			assert.Contains(t, payload, "QuotedBody")
			assert.Equal(t, tt.wantQuotedBody, payload["QuotedBody"])
            assert.Equal(t, replyText, payload["Message"])
		})
	}
}
...
func TestBuildEventPayloadContactsArrayIncludesPhoneNumbers(t *testing.T) {
	...
	contacts, ok := payload["ContactsArray"].([]webhookContactPayload)
	if !ok {
		t.Fatalf("expected ContactsArray to be []webhookContactPayload, got %T", payload["ContactsArray"])
	}
	if len(contacts) != 2 {
