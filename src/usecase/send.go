...
func (service serviceSend) wrapSendMessage(ctx context.Context, client *whatsmeow.Client, recipient types.JID, msg *waE2E.Message, content string, extras ...whatsmeow.SendRequestExtra) (whatsmeow.SendResponse, error) {
	...
	// Trigger manual outgoing webhook for immediate confirmation
	whatsapp.MarkMessageAsSent(ts.ID)
	whatsapp.TriggerWebhookForSentMessage(ctx, client, recipient, msg, ts)
...
func (service serviceSend) SendImage(ctx context.Context, request domainSend.ImageRequest) (response domainSend.GenericResponse, err error) {
	...
	if request.ImageURL != nil && *request.ImageURL != "" {
		...
		// Handle Base64 file
		imageData, err := utils.Base64ToBytes(*request.ImagePath)
		if err != nil {
			return response, pkgError.ValidationError(fmt.Sprintf("failed to decode base64 image: %v", err))
		}
	...
	go func() {
		...
		whatsapp.RunMediaCleanup() // Trigger cleanup
	}()
	if err != nil {
		return response, err
	}
...
func (service serviceSend) SendFile(ctx context.Context, request domainSend.FileRequest) (response domainSend.GenericResponse, err error) {
	...
	if request.FileURL != nil && *request.FileURL != "" {
		...
		// Handle Base64 file
		fileBytes, err = utils.Base64ToBytes(*request.FilePath)
		if err != nil {
			return response, pkgError.ValidationError(fmt.Sprintf("failed to decode base64 file: %v", err))
		}
		if request.FileName != "" {
			fileName = request.FileName
		} else {
			fileName = fmt.Sprintf("upload_%s.bin", fiberUtils.UUIDv4())
		}
	...
	msg := &waE2E.Message{DocumentMessage: &waE2E.DocumentMessage{
		...
		Title:         proto.String(request.Caption), // Use caption as title if provided, otherwise filename
...
func (service serviceSend) SendVideo(ctx context.Context, request domainSend.VideoRequest) (response domainSend.GenericResponse, err error) {
	...
	if request.VideoURL != nil && *request.VideoURL != "" {
		...
		// Handle Base64 video
		videoBytes, err := utils.Base64ToBytes(*request.VideoPath)
		if err != nil {
			return response, pkgError.ValidationError(fmt.Sprintf("failed to decode base64 video: %v", err))
		}
...
func (service serviceSend) SendAudio(ctx context.Context, request domainSend.AudioRequest) (response domainSend.GenericResponse, err error) {
	...
	if request.AudioURL != nil && *request.AudioURL != "" {
		...
		// Handle Base64 audio
		audioBytes, err = utils.Base64ToBytes(*request.AudioPath)
		if err != nil {
			return response, pkgError.ValidationError(fmt.Sprintf("failed to decode base64 audio: %v", err))
		}
		audioFilename = fmt.Sprintf("upload_%s.ogg", fiberUtils.UUIDv4())
		audioMimeType = resolveAudioMIME(audioFilename, audioBytes)
...
func (service serviceSend) SendPoll(ctx context.Context, request domainSend.PollRequest) (response domainSend.GenericResponse, err error) {
	...
	// Save poll data for later decryption
	pollstore.DefaultPollStore.SavePoll(ts.ID, pollstore.PollData{
...
// message. The API receives Unix milliseconds (Date.now() friendly) and
// WhatsApp's PollCreationMessage.endTime is itself in Unix milliseconds, so the
// value is forwarded to the wire unchanged.
//
// The unit was verified empirically: dividing by 1000 (assuming WhatsApp wanted
// seconds, like EventMessage.startTime) makes the poll arrive already closed —
// the app reads the small value as milliseconds in 1970. Forwarding milliseconds
// makes the poll arrive open with the correct countdown.
//
...
func applyPollEndTime(msg *waE2E.Message, endTimeMilliseconds *int64) {
	...
	msg.PollCreationMessage.EndTime = proto.Int64(*endTimeMilliseconds)

	logrus.Infof("Poll scheduled to close at %s (end_time=%d ms on the wire)",
		time.UnixMilli(*endTimeMilliseconds).Format(time.RFC3339), *endTimeMilliseconds)
}
